...
public:
 ...
 private:
    ...
    int ep_epoll_fd;
    set<int> ep_listen_fds;//监听的fd的集合

    typedef unordered_map<int, io_event>::iterator ep_event_map_iter;//ep_event_map的迭代器//
    unordered_map<int, io_event> ep_event_map;//fd和事件的映射
    vector<epoll_event> ep_events;//事件集合
};
